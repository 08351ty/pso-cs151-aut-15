...
;;using this to check functions
(define sample-board1
  (Board (list (Some 'black) (Some 'black) (Some 'black)
               (Some 'white) (Some 'white) (Some 'white)
               (Some 'white) (Some 'white) (Some 'white))
...
;;using this to check functions
(define sample-game1
  (Game sample-board1 'white 'place))
...
(define (compare-symbol a b c d e)
  (match a
    ['none #f]
    [else (cond
            [(and (equal? a b) (equal? a c) (equal? a d) (equal? a e)) #t]
            [else #f])]))
...
(check-expect (compare-symbol (Some 'white) (Some 'white) (Some 'white) 'none (Some 'white)) #f)
...
(define (vertical-win? b)
  (if (or (match-vert (Board-NW b) (Board-SW b))
      ...
      #t
      #f))
(check-expect (vertical-win? sample-board) #f)
...
(define (horizontal-win? b)
  (if (or (match-hori (Board-NW b) (Board-NE b))
      ...
      #t
      #f))
(check-expect (horizontal-win? sample-board) #f)
...
(define (diagonal-win? b)
  (if (or (match-diam1 (Board-NW b) (Board-SE b) (Board-SW b) (Board-NE b))
      ...
      #t
      #f))
(check-expect (diagonal-win? sample-board) #f)
...
(define (game-over? g)
  (if (or (vertical-win? (Game-board g)) (horizontal-win? (Game-board g)) (diagonal-win? (Game-board g)))
      #t
      #f))
(check-expect (game-over? sample-game) #f)
...
(check-expect (outcome sample-game1) 'white)
...
(define (next-player p)
  (match p
    ['white "White"]
    ['black "Black"]))
(check-expect (next-player 'white) "White")
...
(check-expect (next-action sample-game) "Place")
...
;; empty board
(define empty-board
  (Board (make-list 9 'none) (make-list 9 'none) (make-list 9 'none) (make-list 9 'none)))


;; two marbles in quadtrant
(define quad1
...
(define b1
  (Board diag1  (make-list 9 'none)  (make-list 9 'none) diag4))

(define b2
  (Board quad2 quad3 quad4 quad5))

(define game-diag
  (Game b1 'black 'place))

(define game-col
  (Game (Board (make-list 9 'none)  quad2 (make-list 9 'none) quad2) 'black 'place))

(define game-full-tie
  (Game (Board full-list full-list full-list full-list) 'white 'place))

(define game-full-no-win
  (Game (Board no-win1 no-win2 no-win1 no-win2) 'white 'place))

(define game-twist
  (Game b2 'white 'twist))

(define game-twist-sm
  (Game (Board quad2-twist quad3 quad4 quad5) 'black 'place))

(define game-twist-CW
  (Game (Board quad2 quad3-CW quad4 quad5) 'black 'place))

(define game-twist-CCW
  (Game (Board quad2 quad3-CCW quad4 quad5) 'black 'place))

(define game-simple
  (Game (Board quad2 quad2 quad2 quad2) 'black 'place))

(define game-simple-addNW
  (Game (Board quad1 quad2 quad2 quad2) 'black 'twist))

(define game-simple-addNE
  (Game (Board quad2 quad1 quad2 quad2) 'black 'twist))

(define game-simple-addSE
  (Game (Board quad2 quad2  quad1 quad2) 'black 'twist))

(define game-simple-addSW
  (Game (Board quad2 quad2 quad2 quad1) 'black 'twist))

(define game-tie
  (Game (Board quad2-twist tie-quad1 quad4 tie-quad2) 'white 'twist))

(define empty-game
  (Game empty-board 'white 'place))

;; new-game
(check-expect new-game empty-game)

;; board-ref
(check-expect (board-ref b1 (Loc 2 2)) (Some 'black))
(check-expect (board-ref b2 (Loc 1 5)) (Some 'black))
(check-expect (board-ref b2 (Loc 1 4)) (Some 'white))
(check-expect (board-ref b2 (Loc 3 3)) (Some 'black))
(check-expect (board-ref b2 (Loc 3 5)) 'none)

(check-error (board-ref empty-board (Loc 6 0)) "location is invalid")
(check-error (board-ref empty-board (Loc 2 6)) "location is invalid")

;; place-marble
(check-expect (place-marble game-simple 'black (Loc 2 2)) game-simple-addNW)
(check-expect (place-marble game-simple 'black (Loc 2 5)) game-simple-addNE)
(check-expect (place-marble game-simple 'black (Loc 5 2)) game-simple-addSE)
(check-expect (place-marble game-simple 'black (Loc 5 5)) game-simple-addSW)

(check-error (place-marble game-simple 'white (Loc 3 2)) "wrong player is moving")
(check-error (place-marble game-simple 'black (Loc 3 4)) "location is occupied")
(check-error (place-marble game-simple 'black (Loc 6 3)) "move is illegal")
(check-error (place-marble game-simple 'black (Loc 4 6)) "move is illegal")
(check-error (place-marble game-twist 'white (Loc 4 3)) "must twist, cannot place")

;; twist-quadrant
(check-expect (twist-quadrant game-twist 'NW 'clockwise) game-twist-sm)
(check-expect (twist-quadrant game-twist 'NE 'clockwise) game-twist-CW)
(check-expect (twist-quadrant game-twist 'NE 'counterclockwise) game-twist-CCW)

(check-error (twist-quadrant empty-game 'SW 'clockwise) "must place, cannot twist")

;; game-over?
(check-expect (game-over? game-twist) #f)
(check-expect (game-over? game-full-no-win) #t)
(check-expect (game-over? game-full-tie) #t)
(check-expect (game-over? game-diag) #t)
(check-expect (game-over? game-tie) #t)
(check-expect (game-over? empty-game) #f)
(check-expect (game-over? game-simple) #t)

;; outcome
(check-expect (outcome game-full-no-win) 'tie)
(check-expect (outcome game-full-tie) 'tie)
(check-expect (outcome game-tie) 'tie)
(check-expect (outcome game-diag) 'black)
(check-expect (outcome game-simple) 'white)
(check-error (outcome empty-game) "game play not finished")
(check-error (outcome game-twist) "game play not finished")


;; visualization
(board-image empty-board 180)
(board-image empty-board 90)
(board-image empty-board 250)
(board-image (Game-board game-full-no-win) 180)
(board-image (Game-board game-twist) 180)
(board-image (Game-board game-simple) 180)
(board-image (Game-board game-tie) 180)

(game-image game-full-no-win 180)
(game-image game-twist 180)
(game-image game-simple 180)
(game-image game-tie 180)
(game-image empty-game 180)

(test)

(game-image game-simple 150)
(game-image (place-marble game-simple 'black (Loc 2 2)) 150)
(game-image game-simple-addNW 150)

(game-image game-twist 150)
(game-image (twist-quadrant game-twist 'NE 'clockwise) 150)
(game-image game-twist-CW 150)
(game-image (twist-quadrant game-twist 'NE 'counterclockwise) 150)
(game-image game-twist-CCW 150)
...
;; place-marble     5/ 10 not accounting for which player's turn, type of turn, other illegal moves
;; twist-quadrant   9/ 10 not accounting for turn type
;; game-over?       12/ 14 not accounting for tie
;; outcome          8/  8
...
;; adequate tests                     4/ 6 more tests for illegal moves

;; clarity (clear logic)              4/ 6 avoid redundant functions like the separate direction checks
